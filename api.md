...
```bash
LINEAR_API_KEY=lin_api_xxx bun run scripts/query.ts "query { viewer { id name } }"
```
...
LINEAR_API_KEY=lin_api_xxx bun run scripts/sync.ts --issues PROJ-101,PROJ-102,PROJ-103 --state Done

# Preview changes without applying
LINEAR_API_KEY=lin_api_xxx bun run scripts/sync.ts --issues PROJ-101,PROJ-102 --state Done --dry-run

# Add comment with state change
LINEAR_API_KEY=lin_api_xxx bun run scripts/sync.ts --issues PROJ-101 --state Done --comment "Completed in PR #42"
```
...
```bash
LINEAR_API_KEY=lin_api_xxx bun run scripts/query.ts "{ __schema { types { name description } } }"
```
